; Pyrexcoin Cryptocosm GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Monero Project
; See LICENSE

[Setup]
AppName=Pyrexcoin GUI Wallet
...
; As of version 0.14.0.0, the Pyrexcoin .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Pyrexcoin.
...
; Pyrexcoin wallet RPC interface implementation
Source: "bin\pyrexcoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Pyrexcoin daemon
Source: "bin\pyrexcoind.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\pyrexcoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\pyrexcoin-project\pyrexcoin-core', 'blockchainDataDir', blockChainDir);
...
begin
  if CurStep = ssPostInstall then begin
    ...
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Pyrexcoin daemon and then stay with window open after it exits';
...
Name: "{group}\Utilities\Pyrexcoin Daemon"; Filename: "{app}\pyrexcoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\pyrexcoin-wallet-cli.exe"; WorkingDir: "{userdocs}\Pyrexcoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Pyrexcoin\wallets"
...
Root: HKCU; Subkey: "Software\pyrexcoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\pyrexcoin-project\pyrexcoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\pyrexcoin-project\pyrexcoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};
