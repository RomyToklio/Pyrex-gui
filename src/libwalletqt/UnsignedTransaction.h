...
public:
    enum Status {
        Status_Ok       = Pyrexcoin::UnsignedTransaction::Status_Ok,
        Status_Error    = Pyrexcoin::UnsignedTransaction::Status_Error,
        Status_Critical    = Pyrexcoin::UnsignedTransaction::Status_Critical
    };
    Q_ENUM(Status)
...
private:
    explicit UnsignedTransaction(Pyrexcoin::UnsignedTransaction * pt, Pyrexcoin::Wallet *walletImpl, QObject *parent = 0);
    ~UnsignedTransaction();
private:
    friend class Wallet;
    Pyrexcoin::UnsignedTransaction * m_pimpl;
    QString m_fileName;
