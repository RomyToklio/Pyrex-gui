                                    ...
                                    const QString &language, NetworkType::Type nettype)
{
    ...
    Pyrexcoin::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
                                                  language.toStdString(), static_cast<Pyrexcoin::NetworkType>(nettype));
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, NetworkType::Type nettype)
{
    ...
    Pyrexcoin::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), static_cast<Pyrexcoin::NetworkType>(nettype));
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, NetworkType::Type nettype, quint64 restoreHeight)
{
    ...
    Pyrexcoin::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), memo.toStdString(), static_cast<Pyrexcoin::NetworkType>(nettype), restoreHeight);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight)
{
    ...
    Pyrexcoin::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), language.toStdString(), static_cast<Pyrexcoin::NetworkType>(nettype), restoreHeight,
...
{
    return Pyrexcoin::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(Pyrexcoin::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return Pyrexcoin::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return Pyrexcoin::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    return Pyrexcoin::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, NetworkType::Type nettype) const
{
    return Pyrexcoin::Wallet::addressValid(address.toStdString(), static_cast<Pyrexcoin::NetworkType>(nettype));
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  NetworkType::Type nettype) const
{
    std::string error;
    if(!Pyrexcoin::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, static_cast<Pyrexcoin::NetworkType>(nettype), error)){
...
{
    return QString::fromStdString(Pyrexcoin::Wallet::paymentIdFromAddress(address.toStdString(), static_cast<Pyrexcoin::NetworkType>(nettype)));
}
...
{
    return Pyrexcoin::Utils::isAddressLocal(daemon_address.toStdString());
}
...
{
    Pyrexcoin::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    Pyrexcoin::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
QString WalletManager::checkUpdates(const QString &software, const QString &subdir) const
{
  ...
  const std::tuple<bool, std::string, std::string, std::string, std::string> result = Pyrexcoin::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
  if (!std::get<0>(result))
    return QString("");
...
{
    m_pimpl =  Pyrexcoin::WalletManagerFactory::getWalletManager();
}
