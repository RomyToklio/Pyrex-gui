...
{
    // prepare command line arguments and pass to pyrexcoind
    ...
    qDebug() << "starting pyrexcoind " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM pyrexcoind.exe");
#else
                QProcess::execute("pkill pyrexcoind");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./pyrexcoind status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to pyrexcoind
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/pyrexcoind.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/pyrexcoind";
#endif
