...
# Build GUI
        ...
        cd android/docker
        docker build -t pyrexcoin-gui-android .
        docker create -it --name pyrexcoin-gui-android pyrexcoin-gui-android bash

# Get the apk

        docker cp pyrexcoin-gui-android:/opt/android/pyrexcoin-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/pyrexcoin-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
