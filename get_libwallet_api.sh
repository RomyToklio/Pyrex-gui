...
MONERO_DIR=$ROOT_DIR/Pyrex
BUILD_LIBWALLET=false

# init and update Pyrex submodule
if [ ! -d $MONERO_DIR/src ]; then
    git submodule init Pyrex
fi
git submodule update --remote
git -C $MONERO_DIR fetch
git -C $MONERO_DIR checkout release-v0.12.4
...
git -C $MONERO_DIR config user.name "Pyrex GUI"
git -C $MONERO_DIR config user.email "gui@pyrex.local"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging Pyrex push request #$PR"
    # fetch pull request and merge
    git -C $MONERO_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $MONERO_DIR merge --quiet PR-$PR  -m "Merge Pyrex PR #$PR"
...
# Build libwallet if no previous version file exists
    ...
    echo "Pyrex/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove Pyrex/lib/libwallet_merged.a to force rebuild"
    fi
fi
...
echo "cleaning up existing Pyrex build dir, libs and includes"
